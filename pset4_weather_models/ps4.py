...
def r_squared(y, estimated):
    ...
    """
    sum1 = sum([(y-e) ** 2 for y, e in zip(y, estimated)])
    mean = np.mean(y)
    sum2 = sum([(y - mean) ** 2 for y in y])
    return 1 - sum1/sum2

#print(r_squared([1, 2, 8], [1.2, 2.1, 3.9]))
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    x, y = pylab.array(x), pylab.array(y)
    for model in models:
        pylab.plot(x, y, 'bo', label = 'Measured points')
        pylab.title('Evolution of temperature in time')
        pylab.xlabel('Year')
        pylab.ylabel('Temperature')
        #plotting linear fit
        a,b = models[0][0], models[0][1]
        estimated = a * x + b
        pylab.plot(x, estimated, label = 'R^2 = ' + str(r_squared(y, estimated)))
