...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for step in range(time_steps):
            trialResults[step] += patient.update()
    
    avgPop = [trial / float(numTrials) for trial in trialResults]
    pylab.plot(avgPop, label = 'SimpleVirus')
    pylab.title('SimpleVirus population in patient over time')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        #is resistant to all drugs
        effect_drugs = 0
        for drug in activeDrugs:
            if not(self.isResistantTo(drug)):
                effect_drugs += 1
        
        #only reproduces if resistant to all drugs
        if effect_drugs == 0:
             if self.maxBirthProb * (1 - popDensity) > random.random():
                childResistances = {}
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    time_steps = 300
    treatOnStep = 150
    totalResults = time_steps * [0]
    resistResults = time_steps * [0]
    for trial in range(numTrials):
        viruses = []
        for virus in range(numViruses):
        ...
        for step in range(time_steps):
            ...
            patient.update()
            totalResults[step] += patient.getTotalPop()
            resistResults[step] += patient.getResistPop(['guttagonol'])
    

    avgTotalPop = [trial / float(numTrials) for trial in totalResults]
    avgResistPop = [trial / float(numTrials) for trial in resistResults]
    pylab.plot(avgTotalPop, label = 'Total virus population')
    pylab.plot(avgResistPop, label = 'Guttagonol-resistant virus population')
    pylab.title('Total population in patient over time')
    pylab.xlabel('Time steps')
    pylab.ylabel('Average virus population')
    pylab.legend(loc = 'best')
    pylab.show()
